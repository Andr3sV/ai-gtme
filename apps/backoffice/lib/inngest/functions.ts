...
/** Lógica compartida: llama al agente y actualiza la DB. Usado por Inngest y por POST /api/chat en local. */
export async function executeAgentRun(payload: AgentRunPayload): Promise<AgentRunResponse> {
  const { conversation_id, message_id, agent_run_id, content, recent_messages } = payload;
...
export const runAgent = inngest.createFunction(
  ...
  async ({ event }) => {
    const payload: AgentRunPayload = {
      ...
      agent_run_id: event.data.agent_run_id,
      content: event.data.content,
      recent_messages: (event.data as { recent_messages?: Array<{ role: string; content: string }> }).recent_messages,
    };
    return executeAgentRun(payload);
  }
);
